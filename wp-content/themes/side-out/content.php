    ...
    <div class="entry-content">
        <article class="entry">
            <header class="meta-header" id="single-post-header">
                ...
                <h1 class="entry-title" id="post-<?php the_ID(); ?>"><a href="<?php the_permalink() ?>"
                                                                        rel="bookmark"><?php the_title(); ?></a></h1>
            </header>
            <?php if (has_post_thumbnail()) { ?>
                <figure>
                    <?php $image = wp_get_attachment_url(get_post_thumbnail_id($post->ID)); ?>
                    <a href="<?php echo $image; ?>" rel="bookmark"
                       title="<?php the_title(); ?>"><?php the_post_thumbnail(); ?></a></figure>
            <?php } else {
                echo '<div class="no-thumb"></div>';
            } ?>
            <div class="entry-content">
                <?php the_content(); ?>
                <nav class="navigation"><?php wp_link_pages(); ?></nav>
            </div>
