<?php
// Register 'Doppio Recent Posts' widget
add_action('widgets_init', 'init_wp_doppio_recent_posts');

function init_wp_doppio_recent_posts()
{
    return register_widget('wp_doppio_recent_posts');
}

class wp_doppio_recent_posts extends WP_Widget
{
    /** constructor */
    function wp_doppio_recent_posts()
    {
        parent::WP_Widget('wp_doppio_recent_posts', $name = 'Doppio Recent Post');
    }
            ...
            <ul>
                <?php while ($fzq->have_posts()) : $fzq->the_post(); ?>
                    <li class="clearfix">
                        ...
                        <div class="meta-info">
                            <span class="meta-info-date"><?php the_time('F j, Y'); ?></span>, <a
                                href="<?php comments_link(); ?>"
                                class="meta-info-comment"><?php comments_number('0 <i class="fa fa-comments"></i>', '1 <i class="fa fa-comments"></i>', '% <i class="fa fa-comments"></i>'); ?></a>
                        </div>
                    </li>
                    <?php wp_reset_query();
                endwhile; ?>
    ...
    function form($instance)
    {
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e(' Title:', 'wp-doppio'); ?></label>
            <input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>"
                   type="text" value="<?php if (isset($title)) {
                echo $title;
            } ?>" class="widefat"/>
        </p>
        <p>
            <input type="checkbox"
                   name="<?php echo $this->get_field_name('display_featured_image'); ?>" <?php checked($display_featured_image, 1); ?>
                   value="1"/>
            <label
                for="<?php echo $this->get_field_id('display_featured_image'); ?>"><?php _e(' Display Thumbnail:', 'wp-doppio'); ?></label>
        </p>
        <p>
            <label
                for="<?php echo $this->get_field_id('number'); ?>"><?php _e(' Number of posts to show:', 'wp-doppio'); ?></label>
